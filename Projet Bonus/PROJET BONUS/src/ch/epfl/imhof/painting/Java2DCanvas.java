...
public final class Java2DCanvas implements Canvas {
     ...
     * @param res
     *              La résolution du canevas en points par pouce
     * @param color
     *              La couleur de fond du canevas
     * @throws IllegalArgumentException
     *              Si la hauteur, la largeur ou la résolution ne sont pas positives
     */
    public Java2DCanvas(Point bottomLeft, Point topRight, int width, int height, double res, Color color) throws IllegalArgumentException {
        ...
        if (res <= 0)
            throw new IllegalArgumentException("Résolution invalide");
    ...
    /**
     * Retourne un Path2D.Double obtenu à partir de la polyligne passée en argument
     * 
     * @param poly
     * 				La polyligne à convertir
     * @return
    ...
    private Path2D.Double path(PolyLine poly) {
        ...
        if (poly.isClosed()) path.closePath();
        
        return path;
    }
    
    /**
     * Retourne la BufferedImage contenant tous les ajouts faits par le biais du Graphics2D créé dans le constructeur
     * 
     * @return
     *              L'image
     */
